...
public class PlayerMove : MonoBehaviour 
{
	...
	public string joystickJump = "Jump";
	...
	{	
		...
		float h = Input.GetAxisRaw (joystickHorizontal);
		float v = Input.GetAxisRaw (joystickVertical);
			...
			if(Physics.Raycast(check.position, Vector3.down, out hit, dist + 0.05f))
			{
				if(!hit.transform.collider.isTrigger)
				{
					...
					if (hit.transform.tag == "Enemy" && rigidbody.velocity.y < 0)
					{
						...
						onEnemyBounce = 0;
					//moving platforms
					if (hit.transform.tag == "MovingPlatform" || hit.transform.tag == "Player" || hit.transform.tag == "Pushable")
					{
	...
	private void JumpCalculations()
	{
		...
		if (Input.GetButtonDown (joystickJump) && !grounded)
		...
		if (grounded && slope < slopeLimit)
		{
			...
			if (Input.GetButtonDown (joystickJump) || airPressTime + jumpLeniancy > Time.time)
			{	
